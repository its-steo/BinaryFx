...
import { Zap, ShoppingCart, Play, Pause, RefreshCw } from "lucide-react";
...
export default function FxProRobotsPage() {
  ...
  const fetchRobots = useCallback(async () => {
    try {
      ...
      /* ---- Purchased robots ---- */
      const purchasedRobotsList = purchasedRes.data?.user_robots ?? [];   // ← correct key
      ...
      console.error("[v0] Failed to fetch robots:", err);
              ...
              {robots.length > 0 ? (
                robots.map((robot) => {
                  ...
                  return (
                    <div
                      key={robot.id}
                      className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-lg border border-white/10 overflow-hidden"
                    >
                      {robot.image && (
                        <img
                          ...
                          {userRobot.last_trade_time && (
                            <div className="flex justify-between items-center">
                              ...
                              <span className="text-sm">
                                {new Date(userRobot.last_trade_time).toLocaleString()}
                              </span>
                            </div>
                          )}
                        </div>
